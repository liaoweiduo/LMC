...
def get_benchmark(mode, args:ArgsGenerator):
    ...
    """
    from Data.cpin import continual_training_benchmark, fewshot_testing_benchmark
...
def create_dataloader_cpin(benchmark, task_id, args:ArgsGenerator, batch_size=64):
...
def main(args:ArgsGenerator):
    ...
    for i in range(n_tasks):
        ...
        print('==='*10)
        train_loader_current, valid_dataloader, test_loader_current = create_dataloader_cpin(
        ...
        if args.multihead!='none':
            ...
            if i < n_tasks - 1:     # do not increase for the last task
                init_idx=get_oh_init_idx(model, create_dataloader_cpin(continual_train_benchmark, i+1, args, batch_size=args.batch_size)[0], args)
