...
def build_transform_for_vit(img_size=(224, 224), is_train=True):
    if is_train:
        ...
        _train_transform.transforms[0] = transforms.Resize(img_size, interpolation=Image.BICUBIC)
...
def continual_training_benchmark(
    ...
    def obtain_subset(dataset, exp_idx, memory_size=0, transform=None):
        t = dataset.targets
    ...
    val_datasets = [
        obtain_subset(train_set, expidx, 0, eval_transform)
        for expidx in range(n_experiences)
    ]
    test_subsets = [
...
__all__ = ["continual_training_benchmark", 'Benchmark']
